void Electron(){
  ...
  int s, o;
TH1F *hEp[10];
hEp[0] = new TH1F("Ep0.1GeV", "Ep distribution",1000,0,200);
int k[10]={1, 1, 2, 3, 4, 5, 10, 25, 50, 100};
for (s=1; s<10; s++)
{
  o=k[s];
  hEp[s]=new TH1F(Form("Ep%dGeV", o), "Ep distribution",1000,0,200);
}
for (s=1; s<10; s++)
{
...
int j;
TCanvas *c1 = new TCanvas("c1", "c1", 2500, 1000);

TF1 *f[10];
 for (j=0; j<10; j++)
 {
   f[j] = new TF1(TString::Format("f%d",j), "landau");
   //hEp[j]->Fit("landau", "Q");
  int Mbin=hEp[j]->GetMaximumBin();
  double bc=hEp[j]->GetBinCenter(Mbin);
  f[j]->SetParLimits(1, bc-0.5,bc+0.5);
  hEp[j]->Fit(f[j],"","",2, 80);
...
double arr[10]={0.1, 1, 2, 3, 4, 5, 10, 25, 50, 100};
double c;
for (j=0; j<10; j++){
TF1 *g = f[j];
c = g->GetParameter(1);

gr->SetPoint(j, arr[j], c);
gr->SetLineColor(2);
gr->SetLineWidth(4);
gr->SetMarkerColor(4);
gr->SetMarkerStyle(21);
gr->SetTitle("Energy-MPV Graph");
gr->GetXaxis()->SetTitle("ElectronEnergy(GeV)");
gr->GetYaxis()->SetTitle("MPV(MeV)");
